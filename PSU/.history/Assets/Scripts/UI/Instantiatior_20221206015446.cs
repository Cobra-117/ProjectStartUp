...
public class Instantiatior : MonoBehaviour
{
    ...
    private bool lookingForRecipes = Switch.Instance.GetisRecipeSelected();
    ...
    private void Awake() {
        ...
        else
        {
            ...
            firstRecipe.AssignInfo(GetImage(index), restaurant.name, "description");
            Debug.Log("changing first recipe");

            index = restaurantRecommendation.ChooseBestRecipe(user);
            restaurant = Database.LoadRestaurant(index);
            secondRecipe.AssignInfo(GetImage(index), restaurant.name, "description");    
        }
